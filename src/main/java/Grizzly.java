...
public class Grizzly {
    ...
    private static void handleMark(Task[] storedItems, int index, String[] parts) {
        ...
        if (itemToMark < index && itemToMark >= 1) {
            ...
            System.out.println(itemToMark + "." + storedItems[itemToMark - 1].toType() + storedItems[itemToMark - 1].markedBox() + storedItems[itemToMark - 1].toString());
            return;
        }
    ...
    private static void handleList(Task[] storedItems, int index) {
        ...
        for (int i = 1; i < index; i++) {
            System.out.println(i + "." + storedItems[i - 1].toType() + storedItems[i - 1].markedBox() + storedItems[i - 1].toString());
        }
    }
